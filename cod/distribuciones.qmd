...
author: "Lemuel Méndez, Jeikel Navarro, Cristhofer Urrutia"
format: html
editor: visual
...
ggplot(tibble(x = x), aes(x = x)) +
  ...
  labs(
    title = "Portafolio en horizonte de 15 años con ajuste LogNormal",
    subtitle = paste0("Prom = ", round(exp(mu_hat + sd_hat^2 / 2) / 1e6),
                      " | DesvEst = ", round(sqrt((exp(sd_hat^2) - 1) * exp(2*mu_hat + sd_hat^2)) / 1e6)),
    x = "Valor del portafolio (millones)",
    y = "Densidad"
...
cd <- nuevas.acciones$compra.directa
...
ce <- nuevas.acciones$call.europea
