---
title: "Proyecto de Valoración de Instrumentos Financieros I"
author: "Jeikel Navarro Solis, Lemuel Méndez Hidalgo, Cristhofer Urrutia Cascante"
format:
  html:
    toc: true            
    toc-depth: 3        
...
precios.forward <- function(S, T, curva){
  ...
  # se escoje la posicion de vencimiento que sea más cercana
...
modelo.ho.lee <- function(u2, d2, curva, cant.periodos, trayectoria.tasa.corta){
  ...
  # se genera el vector de las proporciones de bajada en el árbol de tasa corta
  dt1v <- k.potencias / ((1 - q) * k.potencias + q)

  # for que recorre cada periodo calculando y guardan la información de esa rama del árbol de tasa corta
...
prob.subida <- function(u, d, rt){
  ...
  # definición cunado se cumplen las condiciones de arbitraje
  prob <- (exp(rt2) - d) / (u - d)
  return(prob)
}
```
...
mercado.acciones <- function(u2, d2, u, d, curva, cant.periodos, valor.accion, tasa.corta){
  ...
  # probabilidad de subir en el árbol de tasa corta
  q <- (1 - d2) / (u2 - d2)

  # vectores para guardar el resultado de la dinamica del mercado (precio de la acción)
  ...
  for(i in 1:cant.periodos){
    ...
    # se calcula la probabilidad de subir en el árbol de acciones
    ...
    # se guarda el valor de la acción
    valor.accion.vec[i + 1] <- valor.accion.vec[i] * ifelse(mov == 1, u, d)

    # se crea una variable booleana que guarda el resultado de si se subió la variable en el árbol de tasa corta
    ...
    # guardar la información de las probabilidades
...
precio.ho.lee <- function(u2, d2, curva, S, T, trayectoria.tasa.corta){
  ...
  ## función de uso local para generar la proporción de bajada de cada periodo
...
acumular.hasta.S <- function(S, venc, valores.tasa.corta){
  ...
  # caso contrario: se extraen los valores del vector de tasa corta que estén entre [tiempo.i + 1, tiempo.f]
  ...
  # factor de acumulación: (1 + R) = e^{sum (rt)}
  return(exp(sum(tasas) * 1/12))
}
```

### Valorar bonos cero cupón
...
valor.bonos <- function(S, curva, venc, valor, u2 = NULL, d2 = NULL, tasa.corta = NULL){
  ...
  ## Se crea un vector que guarde valores de "precios" para los bonos cero cupón
  precios <- numeric(length(venc))

  for(i in seq_along(venc)){

    if(venc[i] < S){
      # ya venció: lo meto a la "cuenta de banco" y acumulo hasta S
      ...
      # aún no vence: precio Ho-Lee condicionado
...
montecarlo <- function(S, u2.CRC, d2.CRC, u.CRC, d.CRC,
  ...
  # vectores para guardar la información de cada simulación
...
ajustar.distribucion <- function(
  ...
  if(length(fits) == 0) stop("Ninguna distribución pudo ajustarse.")
...
graficar.ajuste <- function(
  ...
  subtitulo <- paste0(
    if(!is.null(horizonte)) paste0("Horizonte: ", horizonte, " años | ") else "",
...
estadisticas.riesgo <- function(
  ...
  if(length(x) < 10){
    stop("Muy pocos datos para calcular estadísticas.")
  }
...
### Ajustar distribución del portafolio en el horizonte 4
...
grafica.h4 <- graficar.ajuste(
  ...
  titulo = paste0("Portafolio en horizonte de 4 años con ajuste ", res.h4$mejor),
  horizonte = 4
)
...
### Ajustar distribución del portafolio en el horizonte 7
...
grafica.h7 <- graficar.ajuste(
  ...
  titulo = paste0("Portafolio en horizonte de 7 años con ajuste ", res.h7$mejor),
  horizonte = 7
)
...
### Ajustar distribución del portafolio en el horizonte 10
...
grafica.h10 <- graficar.ajuste(
  ...
  titulo = paste0("Portafolio en horizonte de 10 años con ajuste ", res.h10$mejor),
  horizonte = 10
)
...
### Ajustar distribución del portafolio en el horizonte 15
...
grafica.h15 <- graficar.ajuste(
  ...
  titulo = paste0("Portafolio en horizonte de 15 años con ajuste ", res.h15$mejor),
  horizonte = 15
)
